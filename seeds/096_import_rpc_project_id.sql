-- ============================================================
-- M096: Import RPCs — accept p_project_id parameter
-- ============================================================
-- 081 (import_purchase_invoice) + 086 (import_sales_invoice) Excel
-- import RPC'leri proje atayamıyordu. Faz 1'de project_id kolonu
-- sales_orders + purchase_orders'a eklendi (M095). RPC'ler yeniden
-- imzalanıp insert payload'una project_id eklendi.
--
-- Eski imzaları DROP edip yeni imzayla CREATE (DEFAULT NULL ile en
-- sona eklenmiş p_project_id) — eski client çağrıları hâlâ çalışır
-- çünkü NULL default'a düşer.
-- ============================================================

DROP FUNCTION IF EXISTS public.import_purchase_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text);
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM contacts WHERE id = p_supplier_id AND company_id = p_company_id
  ...
  IF p_warehouse_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM warehouses WHERE id = p_warehouse_id AND company_id = p_company_id AND is_active = true
  ) THEN
    RAISE EXCEPTION 'Warehouse not found for this company';
  END IF;
  IF p_project_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM projects WHERE id = p_project_id AND company_id = p_company_id AND deleted_at IS NULL
  ) THEN
    RAISE EXCEPTION 'Project not found for this company';
  END IF;

  SELECT COALESCE(base_currency, 'KZT') INTO v_base_currency FROM companies WHERE id = p_company_id;
  ...
  INSERT INTO purchase_orders (
    ...
    notes, created_by, is_active, was_invoiced, project_id
  ) VALUES (
    ...
    v_auth_uid, true, true, p_project_id
  ...
  LOOP
    ...
    IF v_is_foreign THEN v_net_up_base := v_net_up_src * v_fx; ELSE v_net_up_base := v_net_up_src; END IF;
    ...
    IF v_is_new OR v_product_id IS NULL THEN
      INSERT INTO products (company_id, name, sku, barcode, unit, cost_price, sale_price, quantity, product_type, is_service, is_active)
      VALUES (p_company_id, v_name, v_sku, v_barcode, COALESCE(v_unit, 'adet'), v_net_up_base, 0, 0, 'raw_material', false, true)
      RETURNING id INTO v_product_id;
      v_products_created := v_products_created + 1;
      v_is_service := false;
    ELSE
      SELECT COALESCE(is_service, false) INTO v_is_service FROM products
       WHERE id = v_product_id AND company_id = p_company_id AND is_active = true;
      IF NOT FOUND THEN RAISE EXCEPTION 'Row % (%): existing_product_id invalid for this company', v_idx, v_name; END IF;
    END IF;

    INSERT INTO purchase_order_items (
      order_id, product_id, description, quantity,
      unit_price, unit_price_foreign, total, total_foreign,
      tax_rate_value, tax_amount, warehouse_id, received_qty
    ) VALUES (
      ...
      COALESCE(p_tax_rate, 0), v_line_tax_base, p_warehouse_id, v_qty
    );
    ...
    IF NOT v_is_service AND p_warehouse_id IS NOT NULL THEN
      INSERT INTO stock_movements (
        ...
        purchase_order_id, reference_type, reference, notes, created_by
      ) VALUES (
  ...
  UPDATE purchase_orders SET subtotal = v_subtotal_base, tax_amount = v_tax_base, total = v_total_base,
  ...
  RETURN jsonb_build_object(
    ...
    'items_count', v_items_inserted, 'products_created', v_products_created,
    'stock_movements_created', v_stock_moved,
    'subtotal', v_subtotal_base, 'tax_amount', v_tax_base, 'total', v_total_base,
...
REVOKE ALL ON FUNCTION public.import_purchase_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.import_purchase_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text, uuid) TO authenticated;

-- ───────────────────────────────────────────────────────
-- import_sales_invoice
-- ───────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.import_sales_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text);
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM contacts WHERE id = p_customer_id AND company_id = p_company_id
  ...
  SELECT COALESCE(base_currency, 'KZT'), COALESCE(allow_negative_stock, false)
    INTO v_base_currency, v_allow_negative FROM companies WHERE id = p_company_id;
  v_fx := COALESCE(p_exchange_rate, 1);
  v_is_foreign := (p_currency_code IS NOT NULL AND p_currency_code <> v_base_currency AND v_fx <> 1);
  v_order_number := COALESCE(NULLIF(TRIM(p_order_number), ''),
  ...
  IF NOT v_allow_negative THEN
    ...
    LOOP
      ...
      v_product_id  := NULLIF(v_item->>'existing_product_id', '')::uuid;
      IF v_is_new OR v_product_id IS NULL THEN
        v_insufficient := v_insufficient || format('%s: 0 mevcut, %s satılıyor (yeni ürün)', COALESCE(v_name,'?'), v_qty);
      ELSE
        ...
        IF NOT v_is_service AND v_existing_qty < v_qty THEN
          v_insufficient := v_insufficient || format('%s: %s mevcut, %s satılıyor', COALESCE(v_name,'?'), v_existing_qty, v_qty);
        END IF;
      END IF;
    END LOOP;
    IF array_length(v_insufficient, 1) > 0 THEN
      RAISE EXCEPTION 'Insufficient stock for % item(s). Toggle "Allow negative stock" or restock first. Details: %',
        array_length(v_insufficient, 1), array_to_string(v_insufficient, ' | ')
  ...
  INSERT INTO sales_orders (
    ...
    issue_date, subtotal, tax_rate, tax_amount, discount, total,
    currency_code, exchange_rate, total_foreign,
    notes, created_by, is_active, was_invoiced, order_type, project_id
  ) VALUES (
    ...
    v_fx, NULL,
    NULLIF(TRIM(COALESCE(p_notes, '')), ''),
    v_auth_uid, true, true, 'sale', p_project_id
  ) RETURNING id INTO v_order_id;
  ...
  LOOP
    ...
    IF p_tax_included AND COALESCE(p_tax_rate, 0) > 0 THEN
      v_net_up_src := v_raw_up / (1 + p_tax_rate / 100);
    ELSE v_net_up_src := v_raw_up; END IF;
    v_tax_up_src := v_net_up_src * COALESCE(p_tax_rate, 0) / 100;
    IF v_is_foreign THEN v_net_up_base := v_net_up_src * v_fx; ELSE v_net_up_base := v_net_up_src; END IF;
    v_line_total_src  := v_qty * v_net_up_src;
    v_line_tax_src    := v_qty * v_tax_up_src;
    ...
    IF v_is_new OR v_product_id IS NULL THEN
      INSERT INTO products (company_id, name, sku, barcode, unit, cost_price, sale_price, quantity, product_type, is_service, is_active)
      VALUES (p_company_id, v_name, v_sku, v_barcode, COALESCE(v_unit, 'adet'), 0, v_net_up_base, 0, 'finished_good', false, true)
      RETURNING id INTO v_product_id;
      v_products_created := v_products_created + 1;
      v_is_service := false;
    ELSE
    ...
    INSERT INTO sales_order_items (
      order_id, product_id, description, quantity,
      unit_price, unit_price_foreign, total, total_foreign,
      tax_rate_value, tax_amount, warehouse_id, discount
    ) VALUES (
      v_order_id, v_product_id, v_description, v_qty,
      v_net_up_base,
      CASE WHEN v_is_foreign THEN v_net_up_src ELSE NULL END,
      v_line_total_base,
      CASE WHEN v_is_foreign THEN v_line_total_src ELSE NULL END,
      COALESCE(p_tax_rate, 0), v_line_tax_base, p_warehouse_id, 0
    );
    v_items_inserted := v_items_inserted + 1;
    ...
    IF NOT v_is_service AND p_warehouse_id IS NOT NULL THEN
      INSERT INTO stock_movements (
        company_id, product_id, warehouse_id, type, quantity,
        sales_order_id, reference_type, reference, notes, created_by
      ) VALUES (
      ...
      v_stock_moved := v_stock_moved + 1;
      UPDATE products p SET quantity = COALESCE((
        SELECT SUM(CASE WHEN sm.type = 'in' THEN sm.quantity ELSE -sm.quantity END)
          FROM stock_movements sm WHERE sm.product_id = p.id AND sm.is_active = true AND sm.deleted_at IS NULL
        ), 0) WHERE p.id = v_product_id;
    END IF;
  ...
  v_total_base := v_subtotal_base + v_tax_base;
  v_total_src  := v_subtotal_src  + v_tax_src;
  UPDATE sales_orders SET subtotal = v_subtotal_base, tax_amount = v_tax_base, total = v_total_base,
    total_foreign = CASE WHEN v_is_foreign THEN v_total_src ELSE NULL END
   WHERE id = v_order_id;
...
REVOKE ALL ON FUNCTION public.import_sales_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.import_sales_invoice(uuid, uuid, text, date, text, numeric, numeric, boolean, uuid, jsonb, text, uuid) TO authenticated;

INSERT INTO public.migrations_log (file_name, notes)
VALUES ('096_import_rpc_project_id.sql',
  'import_purchase_invoice + import_sales_invoice RPC''leri DROP+CREATE ile yeniden imzalandı: p_project_id uuid DEFAULT NULL parametresi en sona eklendi. INSERT payload sales_orders/purchase_orders.project_id''yi set ediyor. Validation: project_id verilmişse aynı şirkete ait + soft-deleted olmayan proje olmalı. Eski client çağrıları (parametre vermeyenler) NULL default ile uyumlu.')
